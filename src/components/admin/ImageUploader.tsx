...
const ImageUploader = ({ value, onChange, label = "Imagem (upload ou link)" }: ImageUploaderProps) => {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const path = `mobility/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        <label className="flex-1">
          ...
          <div className="border border-dashed border-border rounded-md px-3 py-2 text-xs text-center cursor-pointer hover:bg-secondary/50 transition-colors flex items-center justify-center gap-2">
            <Upload size={14} />
            {uploading ? "Enviando..." : "Upload (máx 8MB)"}
          </div>
        </label>
      </div>
      ...
      {value && !uploading && (
        <img
          src={value}
          alt="preview"
          className="rounded-md border border-border max-h-32 object-contain bg-black/30"
        />
      )}
    </div>
  );
